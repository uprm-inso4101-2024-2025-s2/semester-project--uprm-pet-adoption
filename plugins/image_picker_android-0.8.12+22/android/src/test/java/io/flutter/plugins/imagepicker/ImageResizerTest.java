...
class ImageResizer {
  ...
  String resizeImageIfNeeded(
    ...
    try {
      // Fix path splitting for Windows/Linux compatibility
  ...
  private File createFile(File externalFilesDirectory, String child) {
    // Fix path handling for Windows/Linux compatibility
    File image = new File(externalFilesDirectory, child.replace("/", File.separator));
